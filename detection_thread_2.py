...
class DetectionThread(QThread):
    ...
    def __init__(
            ...
            reset_wait_time=None,     # segs para volver a permitir una nueva parada
            roi_norm=None,
            parent=None,
            # IMPORTANTE: en tu modelo, clase=1 => "Advertencia" (basura)
            trash_classes=(1,),     # IDs de clases del MODELO que representan basura
            score_thresh_plc=0.65,   # umbral de score para ACCIONAR PLC
            min_positive_frames=4,  # nº mínimo de frames positivos recientes
    ...
    def _has_trash_detection(self, detections) -> bool:
        ...
        for i in range(1):
            ...
            if classes not in self.trash_classes:
                continue
            if scores < self.score_thresh_plc:
                continue
    ...
    def run(self) -> None:
        ...
        while self._running:
            ...
            # 🔔 Enviar estado de alarma a la GUI                                                                       """ Aqui nuevo """
